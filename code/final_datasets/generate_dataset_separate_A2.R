...
#get dataset for transitions of length 2 and 3 (not stratified, but per random subject)

#load data per subject
dataset_word2<-read.table(paste0("../simple_word_search/A2/",args[2],"/",args[1],"_word2"),header=TRUE,sep=",")
dataset_word3<-read.table(paste0("../simple_word_search/A2/",args[2],"/",args[1],"_word3"),header=TRUE,sep=",")
...
#save data
write.table(dataset_all,file=paste0("~/git/zebrafish_action_sequence_project/processed_data/final_dataset_separated/A2/",args[2],"/",
