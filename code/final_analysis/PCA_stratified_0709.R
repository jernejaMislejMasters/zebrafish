...
full_together[is.na(full_together)]<-0
...
if(condition=="DarkApoLow"){
	...
	predictors<-full_together[,c(-599,-361)]
	
}else if(condition=="DarkApoHigh"){
	
	predictors<-full_together[,c(-838,-503)]
	
}else{
	
	predictors<-full_together[,c(-859,-514)]
	
}
#also remove the redundant counts per length, since proportions are to be used, but leave the total bout count
...
write.table(csv_set_H, file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/healthy_",condition,"set3.csv"),row.names=T,col.names=T)
write.table(csv_set_D, file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/disease_",condition,"set3.csv"),row.names=T,col.names=T)



#save the ranked compounds
write.table(drugs_distances_mean[grep("Dark$",names(drugs_distances_mean))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/mean_in_healthy_",condition,"set3.txt"),row.names=T,col.names=F)
write.table(drugs_distances_std[grep("Dark$",names(drugs_distances_std))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/std_in_healthy_",condition,"set3.txt"),row.names=T,col.names=F)
write.table(drugs_distances_min[grep("Dark$",names(drugs_distances_min))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/min_in_healthy_",condition,"set3.txt"),row.names=T,col.names=F)
write.table(drugs_distances_max[grep("Dark$",names(drugs_distances_max))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/max_in_healthy_",condition,"set3.txt"),row.names=T,col.names=F)

write.table(drugs_distances_mean[-grep("Dark$",names(drugs_distances_mean))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/mean_in_disease_",condition,"set3.txt"),row.names=T,col.names=F)
write.table(drugs_distances_std[-grep("Dark$",names(drugs_distances_std))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/std_in_disease_",condition,"set3.txt"),row.names=T,col.names=F)
write.table(drugs_distances_min[-grep("Dark$",names(drugs_distances_min))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/min_in_disease_",condition,"set3.txt"),row.names=T,col.names=F)
write.table(drugs_distances_max[-grep("Dark$",names(drugs_distances_max))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/max_in_disease_",condition,"set3.txt"),row.names=T,col.names=F)
...
for(drug in drugs){
	...
	png(paste0("~/git/zebrafish_action_sequence_project/results/plots/final_analysis/PCA_plots_stratified/",condition,"/",
					drug,"Control_",condition,"_stratified.png"),width=1500,height=750)
	...
	if(counter_for_order<5){
		...
		plot(PCA1_Drug_Controls[1:13], PCA2_Drug_Controls[1:13],cex=2,col=colrs2,pch=15,xlim=c(-15,25),
				ylim=c(-15,16),xlab="PC1",ylab="PC2",
		...
		leg<-legend("topright", inset=c(-0.32,0), 
				c(paste0("     10 microM ",drug," in healthy,\n     timeframes 0 to 65 minutes\n"),paste0("     1 microM ",drug," in healthy,\n     timeframes 0 to 65 minutes\n"),
						...
						paste0("     10 microM ",drug," in ",state,",\n     timeframes 0 to 65 minutes\n"),paste0("     1 microM ",drug," in ",state,",\n     timeframes 0 to 65 minutes\n"),
						paste0("     3 microM ",drug," in ",state,",\n     timeframes 0 to 65 minutes\n"),paste0("     Control in ",state,",\n     timeframes 0 to 65 minutes\n")), 
		...
		plot(PCA1_Drug_Controls[14:26], PCA2_Drug_Controls[14:26],cex=2,col=colrs2,pch=15,xlim=c(-15,25),
				ylim=c(-15,16),xlab="PC1",ylab="PC2",
				main=paste0("Principal components of ",drug," and control\n in healthy and ",state," assays."))
...
write(order_important_vars,file=paste0("~/git/zebrafish_action_sequence_project/processed_data/final_dataset/B1/PCA_important_variables_",condition,".txt"))
...
write.table(reshaped_together,file=paste0("~/git/zebrafish_action_sequence_project/processed_data/clusters/",condition,"_set3_cluster_scaled_PCA.txt"),row.names=F,col.names=T)

#save as tree
reshaped_together<-as.data.frame(reshaped_together)
rownames(reshaped_together)<-reshaped_together$Group
...
write(hc2Newick(hc),file=paste0(condition,'set3_PCA.newick'))
...
PC1_control_healthy<-pca_model$ind$coord[grep("Control_Dark$",full_together$Group),1]
PC2_control_healthy<-pca_model$ind$coord[grep("Control_Dark$",full_together$Group),2]
png(paste0("~/git/zebrafish_action_sequence_project/results/plots/final_analysis/PCA_plots/",condition,"/All_together_1_microM_",condition,"_in_time_set3.png"),width=1500,height=750)
par(mar=c(5, 5, 5, 20), xpd=TRUE)
plot(PC1_control_healthy[1],PC2_control_healthy[1],xlim=c(xlimit0,xlimit1),ylim=c(ylimit0,ylimit1),pch=19, cex=6, col="grey30", xlab="PC1", ylab="PC2", main=paste0("PCA based plot of all compounds, administered in the low dosage and\nexamined in the ",state," condition, showing change through time"))
arrows(PC1_control_healthy[1],PC2_control_healthy[1],PC1_control_healthy[13],PC2_control_healthy[13],lw=8,col="grey30")

#disease
PC1_control_disease<-pca_model$ind$coord[grep("Control_Dark..*",full_together$Group),1]
PC2_control_disease<-pca_model$ind$coord[grep("Control_Dark..*",full_together$Group),2]
points(PC1_control_disease[1],PC2_control_disease[1], pch=15,cex=6,col="orangered2")
arrows(PC1_control_disease[1],PC2_control_disease[1],PC1_control_disease[13],PC2_control_disease[13],lw=8,col="orangered2")

dose<-"1microM_"
drug_count<-1
for(drug in drugs[-7]){
	
	PC1_control_drug<-pca_model$ind$coord[grep(paste0(drug,dose,"Dark$"),full_together$Group),1]
	PC2_control_drug<-pca_model$ind$coord[grep(paste0(drug,dose,"Dark$"),full_together$Group),2]
	points(PC1_control_drug[1],PC2_control_drug[1], pch=19, cex=4, col=alpha(colrs_all[drug_count], 0.8))
	arrows(PC1_control_drug[1],PC2_control_drug[1],PC1_control_drug[13],PC2_control_drug[13],col=alpha(colrs_all[drug_count], 0.8),lw=2)
	PC1_control_drug_d<-pca_model$ind$coord[grep(paste0(drug,dose,"Dark..*"),full_together$Group),1]
	PC2_control_drug_d<-pca_model$ind$coord[grep(paste0(drug,dose,"Dark..*"),full_together$Group),2]
	points(PC1_control_drug_d[1],PC2_control_drug_d[1], pch=15, cex=4, col=alpha(colrs_all[drug_count], 0.8))
	arrows(PC1_control_drug_d[1],PC2_control_drug_d[1],PC1_control_drug_d[13],PC2_control_drug_d[13],col=alpha(colrs_all[drug_count], 0.8),lw=2)
...
PC1_control_drug<-pca_model$ind$coord[grep(paste0(drug,dose,"Dark$"),full_together$Group),1]
PC2_control_drug<-pca_model$ind$coord[grep(paste0(drug,dose,"Dark$"),full_together$Group),2]
points(PC1_control_drug[1],PC2_control_drug[1], pch=19, cex=4, col=alpha(colrs_all[12], 0.8))
arrows(PC1_control_drug[1],PC2_control_drug[1],PC1_control_drug[13],PC2_control_drug[13],col=alpha(colrs_all[12], 0.8),lw=2)
PC1_control_drug_d<-pca_model$ind$coord[grep(paste0(drug,dose,"Dark..*"),full_together$Group),1]
PC2_control_drug_d<-pca_model$ind$coord[grep(paste0(drug,dose,"Dark..*"),full_together$Group),2]
points(PC1_control_drug_d[1],PC2_control_drug_d[1], pch=15, cex=4, col=alpha(colrs_all[12], 0.8))
arrows(PC1_control_drug_d[1],PC2_control_drug_d[1],PC1_control_drug_d[13],PC2_control_drug_d[13],col=alpha(colrs_all[12], 0.8),lw=2)

leg<-legend("topright", inset=c(-0.25,0), 
		c("1 microM Aripiprazole","1 microM Cariprazine","1 microM Clozapine","1 microM CNO","1 microM Haloperidol","1 microM NDMC","1 microM OSU6162","1 microM PCAP1",
				"1 microM PCAP2","1 microM PCAP3","1 microM PCAP4", "25 microM NDMCHigh", "healthy control", "disease control","healthy","disease induced"), 
...
PC1_control_healthy<-pca_model$ind$coord[grep("Control_Dark$",full_together$Group),1]
PC2_control_healthy<-pca_model$ind$coord[grep("Control_Dark$",full_together$Group),2]
png(paste0("~/git/zebrafish_action_sequence_project/results/plots/final_analysis/PCA_plots/",condition,"/All_together_3_microM_",condition,"_in_time_set3.png"),width=1500,height=750)
par(mar=c(5, 5, 5, 20), xpd=TRUE)
plot(PC1_control_healthy[1],PC2_control_healthy[1],xlim=c(xlimit0,xlimit1),ylim=c(ylimit0,ylimit1),pch=19, cex=6, col="grey30", xlab="PC1", ylab="PC2", main=paste0("PCA based plot of all compounds, administered in the medium dosage and\nexamined in the ",state," condition, showing change through time"))
arrows(PC1_control_healthy[1],PC2_control_healthy[1],PC1_control_healthy[13],PC2_control_healthy[13],lw=8,col="grey30")
...
leg<-legend("topright", inset=c(-0.25,0), 
		c("3 microM Aripiprazole","3 microM Cariprazine","3 microM Clozapine","3 microM CNO","3 microM Haloperidol","3 microM NDMC","3 microM OSU6162","3 microM PCAP1",
				"3 microM PCAP2","3 microM PCAP3","3 microM PCAP4", "50 microM NDMCHigh", "healthy control", "disease control","healthy","disease induced"), 
		col=c(colrs_all, "grey30", "orangered2","black","black"), pch = c(rep(19,times=13), 15, 1, 0),cex=1.7)
...
PC1_control_healthy<-pca_model$ind$coord[grep("Control_Dark$",full_together$Group),1]
PC2_control_healthy<-pca_model$ind$coord[grep("Control_Dark$",full_together$Group),2]
png(paste0("~/git/zebrafish_action_sequence_project/results/plots/final_analysis/PCA_plots/",condition,"/All_together_10_microM_",condition,"_in_time_set3.png"),width=1500,height=750)
par(mar=c(5, 5, 5, 20), xpd=TRUE)
plot(PC1_control_healthy[1],PC2_control_healthy[1],xlim=c(xlimit0,xlimit1),ylim=c(ylimit0,ylimit1),pch=19, cex=6, col="grey30", xlab="PC1", ylab="PC2", main=paste0("PCA based plot of all compounds, administered in the high dosage and\nexamined in the ",state," condition, showing change through time"))
arrows(PC1_control_healthy[1],PC2_control_healthy[1],PC1_control_healthy[13],PC2_control_healthy[13],lw=8,col="grey30")
...
leg<-legend("topright", inset=c(-0.25,0), 
		c("10 microM Aripiprazole","10 microM Cariprazine","10 microM Clozapine","10 microM CNO","10 microM Haloperidol","10 microM NDMC","10 microM OSU6162","10 microM PCAP1",
				"10 microM PCAP2","10 microM PCAP3","10 microM PCAP4", "100 microM NDMCHigh", "healthy control", "disease control","healthy","disease induced"), 
		col=c(colrs_all, "grey30", "orangered2","black","black"), pch = c(rep(19,times=13), 15, 1, 0),cex=1.7)
...
write.table(csv_set_H, file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/healthy_",condition,"set4.csv"),row.names=T,col.names=T)
write.table(csv_set_D, file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/disease_",condition,"set4.csv"),row.names=T,col.names=T)


#save the ranked compounds
write.table(drugs_distances_mean[grep("Dark$",names(drugs_distances_mean))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/mean_in_healthy_",condition,"set4.txt"),row.names=T,col.names=F)
write.table(drugs_distances_std[grep("Dark$",names(drugs_distances_std))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/std_in_healthy_",condition,"set4.txt"),row.names=T,col.names=F)
write.table(drugs_distances_min[grep("Dark$",names(drugs_distances_min))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/min_in_healthy_",condition,"set4.txt"),row.names=T,col.names=F)
write.table(drugs_distances_max[grep("Dark$",names(drugs_distances_max))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/max_in_healthy_",condition,"set4.txt"),row.names=T,col.names=F)

write.table(drugs_distances_mean[-grep("Dark$",names(drugs_distances_mean))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/mean_in_disease_",condition,"set4.txt"),row.names=T,col.names=F)
write.table(drugs_distances_std[-grep("Dark$",names(drugs_distances_std))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/std_in_disease_",condition,"set4.txt"),row.names=T,col.names=F)
write.table(drugs_distances_min[-grep("Dark$",names(drugs_distances_min))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/min_in_disease_",condition,"set4.txt"),row.names=T,col.names=F)
write.table(drugs_distances_max[-grep("Dark$",names(drugs_distances_max))],
		file=paste0("~/git/zebrafish_action_sequence_project/results/output/ranked_drugs_PCA/max_in_disease_",condition,"set4.txt"),row.names=T,col.names=F)
...
for(drug in drugs){
	...
	png(paste0("~/git/zebrafish_action_sequence_project/results/plots/final_analysis/PCA_plots_turn_only_stratified/",condition,"/",
					drug,"Control_",condition,"_turn_only_stratified.png"),width=1500,height=750)
	...
	if(counter_for_order<5){
		...
		#1:13 is 10microM in healthy
		plot(PCA1_Drug_Controls[1:13], PCA2_Drug_Controls[1:13],cex=2,col=colrs2,pch=15,xlim=c(-15,25),
				ylim=c(-15,20),xlab="PC1",ylab="PC2",
				main=paste0("Principal components of ",drug," and control\n in healthy and ",state," assays."))
...
#save the list of most important variables in order
order_important_vars<-names(rev(apply(pca_model$var$contrib,1,sum)[order(apply(pca_model$var$contrib,1,sum))]))
write(order_important_vars,file=paste0("~/git/zebrafish_action_sequence_project/processed_data/final_dataset/B1/PCA_important_variables_",condition,"_turn_only.txt"))
...
reshaped_together<-cbind(reshaped_together[,1],scale(reshaped_together[,-1]))
colnames(reshaped_together)[1]<-"Group"
write.table(reshaped_together,file=paste0("~/git/zebrafish_action_sequence_project/processed_data/clusters/",condition,"_set4_cluster_scaled_PCA.txt"),row.names=F,col.names=T)
...
write(hc2Newick(hc),file=paste0(condition,'set4_PCA.newick'))

#make vector plots per dose, otherwise unvisible

#first plot the controls
ma5 = c(1, 1, 1)/3#filter
#healthy
PC1_control_healthy<-pca_model$ind$coord[grep("Control_Dark$",full_together$Group),1]
PC2_control_healthy<-pca_model$ind$coord[grep("Control_Dark$",full_together$Group),2]
png(paste0("~/git/zebrafish_action_sequence_project/results/plots/final_analysis/PCA_plots/",condition,"/All_together_1_microM_",condition,"_in_time_set4.png"),width=1500,height=750)
par(mar=c(5, 5, 5, 20), xpd=TRUE)
plot(PC1_control_healthy[1],PC2_control_healthy[1],xlim=c(xlimit0,xlimit1),ylim=c(ylimit0,ylimit1),pch=19, cex=6, col="grey30", xlab="PC1", ylab="PC2", main=paste0("PCA based plot of all compounds, administered in the low dosage and\nexamined in the ",state," condition, showing change through time"))
arrows(PC1_control_healthy[1],PC2_control_healthy[1],PC1_control_healthy[13],PC2_control_healthy[13],lw=8,col="grey30")
...
PC1_control_healthy<-pca_model$ind$coord[grep("Control_Dark$",full_together$Group),1]
PC2_control_healthy<-pca_model$ind$coord[grep("Control_Dark$",full_together$Group),2]
png(paste0("~/git/zebrafish_action_sequence_project/results/plots/final_analysis/PCA_plots/",condition,"/All_together_3_microM_",condition,"_in_time_set4.png"),width=1500,height=750)
par(mar=c(5, 5, 5, 20), xpd=TRUE)
plot(PC1_control_healthy[1],PC2_control_healthy[1],xlim=c(xlimit0,xlimit1),ylim=c(ylimit0,ylimit1),pch=19, cex=6, col="grey30", xlab="PC1", ylab="PC2", main=paste0("PCA based plot of all compounds, administered in the medium dosage and\nexamined in the ",state," condition, showing change through time"))
arrows(PC1_control_healthy[1],PC2_control_healthy[1],PC1_control_healthy[13],PC2_control_healthy[13],lw=8,col="grey30")
...
PC1_control_healthy<-pca_model$ind$coord[grep("Control_Dark$",full_together$Group),1]
PC2_control_healthy<-pca_model$ind$coord[grep("Control_Dark$",full_together$Group),2]
png(paste0("~/git/zebrafish_action_sequence_project/results/plots/final_analysis/PCA_plots/",condition,"/All_together_10_microM_",condition,"_in_time_set4.png"),width=1500,height=750)
par(mar=c(5, 5, 5, 20), xpd=TRUE)
plot(PC1_control_healthy[1],PC2_control_healthy[1],xlim=c(xlimit0,xlimit1),ylim=c(ylimit0,ylimit1),pch=19, cex=6, col="grey30", xlab="PC1", ylab="PC2", main=paste0("PCA based plot of all compounds, administered in the high dosage and\nexamined in the ",state," condition, showing change through time"))
arrows(PC1_control_healthy[1],PC2_control_healthy[1],PC1_control_healthy[13],PC2_control_healthy[13],lw=8,col="grey30")
